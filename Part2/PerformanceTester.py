...
class PerformanceTester:
    ...
    def compute_map_10(self):
        ...
        for k in range(len(queries)):
            for l in range(5):
                if nb_queries >= self.nb_queries:  # Limit of test queries exceeded
                    break
                nb_queries += 1

                print(">>> Calculating the average precision (AP) for query number " + str(nb_queries))
                query = queries[k][1][l]
                query_embedding = np.sign(self.model.caption_model(np.array([query, ])))

                features_score = []
                for f in concatenated_captions_and_features:
                    feature_embedding = np.sign(self.model.image_model(np.array([f[2], ], dtype="float32")))
                    distance = hamming(query_embedding[0], feature_embedding[0])
                    features_score.append((distance, f[0], f[1]))
                features_score = sorted(features_score, key=lambda tup: tup[0], reverse=False)[:10]
